...
// ---- Local notification simulation (remove before Store distribution) --------
let notificationTestBusy = false;
let notificationTestRefresh = 0;
function renderNotificationTest(result) {
  const out = document.getElementById("notificationTestStatus");
  if (!result?.success) { out.textContent = result?.error || "Test status unavailable."; return; }
  const s = result.test || {};
  document.getElementById("toolbarTestNotice").hidden = s.indicator !== true;
  const time = value => new Date(value).toLocaleTimeString();
  let text;
  switch (s.phase) {
    case "scheduled": text = `Scheduled for approximately ${time(s.dueAt)}. You can close the popup now.`; break;
    case "sending": text = "Requesting test notification..."; break;
    case "requested": text = `Chrome accepted the notification request at ${time(s.requestedAt)}. This does not confirm a visible banner.`; break;
    case "blocked": case "failed": case "interrupted": text = s.error; break;
    case "cleared": text = "Test cancelled; test icon marker and notification cleared. A real pending update keeps its own marker."; break;
    default: text = "No test scheduled.";
  }
  if (s.indicator) text += " Test toolbar marker is active until you clear it, even if the desktop notification is blocked.";
  if (s.delayed) text += " This was a delayed/recovered test.";
  if (s.clickedAt) text += " Notification click received.";
  if (s.opened) text += ` Opened SORT ${s.opened}.`;
  if (s.openError) text += " Could not open SORT: " + s.openError;
  if (s.closedAt) text += s.closedByUser ? " Notification dismissed by user." : " Notification closed by the system.";
  text += ` Chrome notification permission: ${result.permission || "unknown"}.`;
  if (result.permission === "denied") text += " Enable notifications before testing.";
  out.textContent = text;
}
async function refreshNotificationTest() {
  if (notificationTestBusy) return;
  const request = ++notificationTestRefresh;
  try {
    const result = await chrome.runtime.sendMessage({type: "getNotificationTestStatus"});
    if (request === notificationTestRefresh && !notificationTestBusy) renderNotificationTest(result);
  } catch (_) {
    if (request === notificationTestRefresh && !notificationTestBusy)
      renderNotificationTest({success: false, error: "Cannot reach SORT. Reopen this test build's popup."});
  }
}
async function notificationTestAction(type) {
  if (notificationTestBusy) return;
  notificationTestBusy = true;
  ++notificationTestRefresh;
  const schedule = document.getElementById("scheduleNotificationTest");
  const clear = document.getElementById("clearNotificationTest");
  schedule.disabled = clear.disabled = true;
  document.getElementById("notificationTestStatus").textContent = type === "scheduleNotificationTest"
    ? "Scheduling background test..." : "Clearing test...";
  try { renderNotificationTest(await chrome.runtime.sendMessage({type})); }
  catch (_) { renderNotificationTest({success: false, error: "Test command failed. Reopen SORT to check its saved status."}); }
  finally { notificationTestBusy = false; schedule.disabled = clear.disabled = false; }
}
document.getElementById("scheduleNotificationTest").addEventListener("click", () => notificationTestAction("scheduleNotificationTest"));
document.getElementById("clearNotificationTest").addEventListener("click", () => notificationTestAction("clearNotificationTest"));
if (new URLSearchParams(location.search).has("notificationTest")) {
  showSettingsTab();
  document.getElementById("notificationTestPanel").scrollIntoView({block: "center"});
}
void refreshNotificationTest();
setInterval(() => { if (!document.hidden) void refreshNotificationTest(); }, 1000);
